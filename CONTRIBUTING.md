# Contributing to GhostShift OS

When contributing to GhostShift OS, make sure that the changes you wish to make are in line with the project direction. If you are not sure about this, open an issue first, so we can discuss it.
...
Join our Discord server or check the GitHub repository: [GhostShift OS](https://github.com/ayu-haker/GhostShift-OS)

## Issue policy

Unlike many other software projects, GhostShift OS is not concerned with gaining the largest possible userbase. Its target audience is its own developers. As such, we have limited interest in feature requests from non-contributors.
...
In GhostShift OS, we treat human language as seriously as we do programming language.
...
-   Write in idiomatic GhostShift OS C++23, using the `AK` containers in all code.
-   Conform to the project coding style found in [CodingStyle.md](https://github.com/ayu-haker/GhostShift-OS/blob/master/Documentation/CodingStyle.md). Use `clang-format` (version 20 or later) to automatically format C++ files. See [AdvancedBuildInstructions.md](https://github.com/ayu-haker/GhostShift-OS/blob/master/Documentation/AdvancedBuildInstructions.md#clang-format-updates) for instructions on how to get an up-to-date version if your OS distribution does not ship clang-format-20.
